...
export const BeforeAfterSlider: React.FC<BeforeAfterSliderProps> = ({
  ...
  return (
    <div className="relative w-full h-64 rounded-2xl overflow-hidden shadow-lg select-none group border border-slate-200 dark:border-slate-800">
      ...
      <div 
        className={`absolute inset-0 bg-gradient-to-br ${afterColor} flex flex-col justify-between p-6 text-white bg-cover bg-center z-10`}
        style={afterImage ? { backgroundImage: `url(${afterImage})` } : {}}
      >
        {afterImage && <div className="absolute inset-0 bg-black/40 z-0" />}
        ...
        <div className="z-10 relative text-right flex flex-col items-end max-w-[48%] ml-auto">
          <h4 className="text-lg md:text-xl font-bold drop-shadow-md truncate w-full">{afterLabel}</h4>
          <p className="text-white/80 text-[10px] md:text-xs mt-1">Drag the slider to see the transformation</p>
        </div>
      </div>

      {/* BEFORE CONTAINER (Foreground, clipped based on slider position) */}
      <div
        className={`absolute inset-0 bg-gradient-to-br ${beforeColor} flex flex-col justify-between p-6 text-white overflow-hidden bg-cover bg-center z-20`}
        style={{ 
          clipPath: `polygon(0 0, ${sliderPosition}% 0, ${sliderPosition}% 100%, 0 100%)`,
          ...(beforeImage ? { backgroundImage: `url(${beforeImage})` } : {})
        }}
      >
        {beforeImage && <div className="absolute inset-0 bg-black/40 z-0" />}
        ...
        <div className="z-10 relative text-left flex flex-col items-start max-w-[48%] mr-auto">
          <h4 className="text-lg md:text-xl font-bold text-slate-100 truncate w-full">{beforeLabel}</h4>
          <p className="text-slate-300 text-[10px] md:text-xs mt-1">Ready for creative reuse</p>
        </div>
      </div>

      {/* DRAGGABLE SLIDER LINE AND HANDLE */}
      <div
        className="absolute top-0 bottom-0 w-1 bg-white cursor-ew-resize pointer-events-none z-30 flex items-center justify-center shadow-lg"
